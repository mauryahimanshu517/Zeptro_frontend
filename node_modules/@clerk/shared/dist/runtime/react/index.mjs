...
import { ClerkRuntimeError, isClerkAPIResponseError } from "../error-B2RA62bv.mjs";
import "../noop-8bk75rVj.mjs";
...
import { eventMethodCalled } from "../telemetry-x9LpQpc1.mjs";
import React, { useCallback, useEffect, useMemo, useRef, useState, useSyncExternalStore } from "react";
import { Fragment, jsx, jsxs } from "react/jsx-runtime";
import useSWR, { default as useSWR$1 } from "swr";
...
import * as import_swr from "swr";
__reExport(clerk_swr_exports, import_swr);
...
const OrganizationProvider = ({ children, organization, swrConfig }) => {
	return /* @__PURE__ */ jsx(clerk_swr_exports.SWRConfig, {
		value: swrConfig,
...
const usePagesOrInfinite = (params, fetcher, config, cacheKeys) => {
	...
	const { data: swrData, isValidating: swrIsValidating, isLoading: swrIsLoading, error: swrError, mutate: swrMutate } = useSWR$1(swrKey, swrFetcher, {
		keepPreviousData,
		...cachingSWROptions
	});
...
function useUser() {
	...
	const user = useUserContext();
	const clerk = useClerkInstanceContext();
	clerk.telemetry?.record(eventMethodCalled(hookName$1));
...
const useSubscription = (params) => {
	...
	const swr = useSWR$1(billingEnabled ? {
...
const Elements = ({ stripe: rawStripeProp, options, children }) => {
	...
	return /* @__PURE__ */ jsx(ElementsContext.Provider, {
		value: ctx,
		children
	});
};
...
const createElementComponent = (type, isServer$1) => {
	...
	const ClientElement = ({ id, className, fallback, options = {}, onBlur, onFocus, onReady, onChange, onEscape, onClick, onLoadError, onLoaderStart, onNetworksChange, onConfirm, onCancel, onShippingAddressChange, onShippingRateChange }) => {
		...
		return /* @__PURE__ */ jsxs(Fragment, { children: [!isReady && fallback, /* @__PURE__ */ jsx("div", {
			id,
			style: {
...
const StripeLibsProvider = ({ children }) => {
	...
	return /* @__PURE__ */ jsx(StripeLibsContext.Provider, {
...
const ValidateStripeUtils = ({ children }) => {
	...
	return /* @__PURE__ */ jsx(StripeUtilsContext.Provider, {
		value: { value: {
			stripe,
			elements
		} },
...
const PropsProvider = ({ children,...props }) => {
	...
	return /* @__PURE__ */ jsx(PaymentElementContext.Provider, {
		value: { value: {
			...props,
			...utils,
...
const PaymentElementProvider = ({ children,...props }) => {
	return /* @__PURE__ */ jsx(StripeLibsProvider, { children: /* @__PURE__ */ jsx(PropsProvider, {
		...props,
		children: /* @__PURE__ */ jsx(PaymentElementInternalRoot, { children })
	}) });
};
const PaymentElementInternalRoot = (props) => {
	const { stripe, externalClientSecret, stripeAppearance } = usePaymentElementContext();
	const locale = useLocalization();
	if (stripe && externalClientSecret) return /* @__PURE__ */ jsx(Elements, {
		stripe,
		options: {
			loader: "never",
		...
		children: /* @__PURE__ */ jsx(ValidateStripeUtils, { children: props.children })
	}, externalClientSecret);
	return /* @__PURE__ */ jsx(DummyStripeUtils, { children: props.children });
};
const PaymentElement = ({ fallback }) => {
	...
	if (!stripe || !externalClientSecret) return /* @__PURE__ */ jsx(Fragment, { children: fallback });
	return /* @__PURE__ */ jsx(PaymentElement$1, {
		fallback,
		onReady,
		options
	});
